...
//	int a = (int)(*g);
	...
	if (i_recite == 1)//(tolower(i_recite) == (int)'r')
	{
		...
		printf("%d", result);
		printf("\n La dirección de memoria la función f_fibonacci: %p", f_fibonacci);
		printf("\n La dirección de memoria la función f_fibonacci_rec: %p", f_fibonacci_ite);
	}
	else if (i_recite == 2)//(tolower(i_recite) == (int)'s')
	{
		//Llamada a función Fibonacci con puntero a función
		//	NO olvidar hacer el casting (int (*)(const int))
		int result = f_fibonacci((int (*)(const int)) f_fibonacci_ite, i_orden);
		printf("\n %d", result );
		printf("\n La dirección de memoria la función f_fibonacci: %p", f_fibonacci);
		printf("\n La dirección de memoria la función f_fibonacci_ite: %p", f_fibonacci_ite);
	}
	else
	{
...
//		printf("\n");
//		printf("%d \t | ", i_i);
//		//printf("%d", f_fibonacci(const e_recite_t n, int (*f)(const int n)));
//	}
...
int f_fibonacci(int (*f)(const int fn), const int n)
{
	printf("\n [f_fibonacci]La dirección de memoria la función f_fibonacci: %p", f_fibonacci);
	printf("\n [f_fibonacci]La dirección de memoria la función f: %p", f);
	return (*f)(n);
}
