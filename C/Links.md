C Links
======

General
Memory
TCP/IP
Memory leaks
Tools
Call graph
Pattern designs

General
* [Yo Linux](http://www.yolinux.com)
* [C con clase](http://c.conclase.net)
* http://publications.gbdirect.co.uk/c_book/
* http://www.elrincondelc.com/nuevorincon/index.php

Pattern Designs
* [Why use double pointer?](http://stackoverflow.com/questions/5580761/why-use-double-pointer-or-why-use-pointers-to-pointers)
* [Wikipedia - Opaque Pointer](http://en.wikipedia.org/wiki/Opaque_pointer#C)
* [hiding private member in header file](http://cboard.cprogramming.com/cplusplus-programming/51227-hiding-private-member-header-file.html)
* [Abstract data types in C](http://inst.eecs.berkeley.edu/~selfpace/studyguide/9C.sg/Output/ADTs.in.C.html)

Memory
* [Almacenamiento, aspectos físicos y lógicos](http://www.zator.com/Cpp/E2_2_6.htm)
* [Memory Structure of a C programme](http://shareprogrammingtips.com/structure-of-a-c-program-in-memory-how-heapstackdata-and-code-segments-are-stored-in-memory/)
* [TCP/IP, Networking, traffic injection. Send an arbitrary Ethernet frame using an AF_PACKET socket in C](http://www.microhowto.info/howto/send_an_arbitrary_ethernet_frame_using_an_af_packet_socket_in_c.html)

Threads
	POSIX Threads Programming
	https://computing.llnl.gov/tutorials/pthreads/

	POSIX thread (pthread) libraries
	http://www.yolinux.com/TUTORIALS/LinuxTutorialPosixThreads.html

	POSIX threads tutorial
	http://www.mobydisk.com/softdev/techinfo/pthreads_tutorial/

	POSIX Threads Tutorial
	http://math.arizona.edu/~swig/documentation/pthreads/#pthreads

	Algunos detalles sobre los Threads
	http://www.chuidiang.com/clinux/procesos/mashilos.php

	Threads (Procesos ligeros) c++ y linux - MUTEX -
	http://www.svcommunity.org/forum/cc/threads-%28procesos-ligeros%29-c-y-linux-mutex/

	C++ Misc: Creating a Thread
	http://www.youtube.com/watch?v=8C4wLzp5lK4

	Grupo 12 - Video Linux - Procesos y Threads (parte 2).avi
	http://www.youtube.com/watch?v=eeXP6iVHnzE

Memory leaks
	how to prevent memory leaks - Buscar con Google
	http://www.google.es/#sclient=psy&hl=es&q=how+to+prevent+memory+leaks&aq=f&aqi=&aql=&oq=&pbx=1&fp=1&bav=on.2,or.r_gc.r_pw.,cf.osb&cad=b

	How to Prevent Memory Leaks in C++ | eHow.com
	http://www.ehow.com/how_2190610_prevent-memory-leaks-c.html

	Memory leak - Wikipedia, the free encyclopedia
	http://en.wikipedia.org/wiki/Memory_leak

Tools
	Librerías C/C++:
	http://www.zator.com/libreriasC.htm

	Realizar Instaladores de forma sencilla con NSIS 2.0 (Nullsoft Scriptable Install System 2.0)
	http://www.adictosaltrabajo.com/tutoriales/tutoriales.php?pagina=nsis

	Señales y Alarmas en C de Unix/Linux
	http://www.chuidiang.com/clinux/senhales/senhales.php

	signal.h - Wikipedia, the free encyclopedia
	http://en.wikipedia.org/wiki/Signal.h

	Preprocesador de C - Wikipedia, la enciclopedia libre
	http://es.wikipedia.org/wiki/Preprocesador_de_C

	A Library of Software written in C with full source code
	http://cplus.about.com/od/codelibraryforc/A_Library_of_Software_written_in_C_with_full_source_code.htm

Call graph
    https://en.wikipedia.org/wiki/Call_graph

    Tool to track #include dependencies
    http://stackoverflow.com/questions/42308/tool-to-track-include-dependencies

    Automatically discovering C dependencies
    http://stackoverflow.com/questions/204823/automatically-discovering-c-dependencies
    https://en.wikipedia.org/wiki/Opaque_pointer#C

C Online Test:
    http://www.pskills.org/testresult.jsp

    http://www.indiabix.com/online-test/c-programming-test/
