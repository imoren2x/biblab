C++ links
======

OOP
Templates
Exceptions
Pattern Designs
UML
Libraries
Process/threads
Databases
Channels (video tutorial and such)
Polymorphism
Memory Leaks
Tools
Misc

* [Manual C++ Zator](http://www.zator.com/Cpp/E_Ce.htm)


OOP
* [YouTube - C++ Console Lesson 18: Arguments and Return Values](http://www.youtube.com/watch?v=8E_Tdu8E1IY)

* [Programación en C++/Sobrecarga de Operadores - Wikilibros](http://es.wikibooks.org/wiki/Programaci%C3%B3n_en_C%2B%2B/Sobrecarga_de_Operadores)

* [Inheritance, what your mother never told you](http://www.parashift.com/c++-faq-lite/strange-inheritance.html)

    OopDocbook Main Page
    http://cartan.cas.suffolk.edu/oopdocbook/

    C++ Programming/Code/Design Patterns - Wikibooks, open books for an open world
    http://en.wikibooks.org/wiki/C++_Programming/Code/Design_Patterns

Templates
    http://www.cplusplus.com/doc/tutorial/templates/

Exceptions
    YouTube - C++ Tutorial - 29 - Exception Handling
    http://www.youtube.com/watch?v=01b_TFpcJ28

    Tratamiento de excepciones
    http://www.zator.com/Cpp/E1_6.htm

Pattern Designs
    C++ Programming/Code/Design Patterns - Wikibooks, open books for an open world
    http://en.wikibooks.org/wiki/C++_Programming/Code/Design_Patterns#Structural_Patterns

    Singleton Design Pattern in C++
    http://sourcemaking.com/design_patterns/singleton/cpp/2

    Singleton Design Pattern in C++: Before and after
    http://sourcemaking.com/design_patterns/singleton/cpp/1

    Singleton - Wikipedia, la enciclopedia libre
    http://es.wikipedia.org/wiki/Singleton

    Logging in C++
    http://drdobbs.com/cpp/201804215

UML
    23405 Activity and Sequence Diagrams from the Source Code on C++
    http://cc.embarcadero.com/Item/23405

    Sequence Diagram Software at FilesLand
    http://www.filesland.com/software/sequence-diagram.html

    Mscgen
    http://www.mcternan.me.uk/mscgen/

    Object diagram - Wikipedia, the free encyclopedia
    http://en.wikipedia.org/wiki/Object_diagram

    UML: Casos de Uso. Análisis y Diseño. Ingenieria del Software.
    http://www.ingenierosoftware.com/analisisydiseno/casosdeuso.php

    Elementos de UML
    http://docs.kde.org/stable/es/kdesdk/umbrello/uml-elements.html

    Diagrama de casos de uso - Wikipedia, la enciclopedia libre
    http://es.wikipedia.org/wiki/Diagrama_de_casos_de_uso

    Visual Paradigm for UML (Community Edition) for Linux Software Developer Editors and Tools - Free Software Download
    http://www.lisisoft.com/free-download-version/152172-visual-paradigm-for-uml-community-edition-for-linux.html

Libraries
    Creación de librerías
    http://arco.esi.uclm.es/~david.villa/doc/repo/librerias/librerias.html

    Librerías estáticas y dinámicas
    http://www.chuidiang.com/clinux/herramientas/librerias.php

Process/threads
    Thread Programming in Windows
    http://www.cplusplus.com/forum/windows/1077/

    Creating Threads (Windows)
    http://msdn.microsoft.com/en-us/library/ms682516(v=vs.85).aspx

    Threads: Basic Theory and Libraries
    http://www.cs.cf.ac.uk/Dave/C/node29.html

    Threads in C++
    http://www.linuxselfhelp.com/HOWTO/C++Programming-HOWTO-18.html

    Use shared objects on Linux
    http://www.ibm.com/developerworks/linux/library/l-shared.html

    C++ Multithreading
    http://www.tutorialspoint.com/cplusplus/cpp_multithreading.htm


Databases
    Acceso a MySQL desde Visual C++
    http://www.latindevelopers.com/articulos/visualc/mysql-api.php

    Conectar a SQL Server con C++
    http://www.elrincondelc.com/nuevorincon/foros/viewtopic.php?p=50323&sid=dae86bbf7c0d3329de641c98078b57e3

Channels
    YouTube - Canal de antiRTFM
    http://www.youtube.com/user/antiRTFM#g/c/1D10C030FDCE7CE0

    YouTube - Canal de ProgrammingVideos
    http://www.youtube.com/user/ProgrammingVideos

## Polymorphism
* [Polimorfismo vs sobrecarga](https://www.zator.com/Cpp/E4_11_8aw1.htm)

* [Polimorfismo](https://www.zator.com/Cpp/E4_11_8.htm)

* https://es.cppreference.com/w/cpp/language/override 

Memory leaks
    How to prevent memory leaks
    http://www.google.es/#sclient=psy&hl=es&q=how+to+prevent+memory+leaks&aq=f&aqi=&aql=&oq=&pbx=1&fp=1&bav=on.2,or.r_gc.r_pw.&cad=b

    How to prevent memory leaks in C++
    http://www.ehow.com/how_2190610_prevent-memory-leaks-c.html

    Memory leak
    http://en.wikipedia.org/wiki/Memory_leak

Tools
    YouTube - C++ Console Lesson 22: Using the rand() Function
    http://www.youtube.com/watch?v=PZZUJ1fxG04&playnext_from=TL&videos=x4T2Llg2JAg&feature=rec-LGOUT-exp_stronger_r2-2r-2-HM

    Integrated Development Environment: C/C++ development with the Eclipse Platform
    http://www.ibm.com/developerworks/opensource/library/os-ecc/

    YouTube - C++ Windows API tutorial 2: Creating a window
    http://www.youtube.com/watch?v=1AaC7-AYFaQ&playnext_from=TL&videos=pPFkvcKe6tc&feature=rec-LGOUT-exp_stronger_r2-2r-1-HM

    Core dump
    http://en.wikipedia.org/wiki/Core_dump

    http://www.kernel.org/doc/man-pages/online/pages/man5/core.5.html

Misc
    Re: packet interceptor
    you can use pcap library to write a one.
    http://en.wikipedia.org/wiki/Pcap
    two semesters ago I had done a project related to this, under network programming.

    This may be those example samples that you looking for.
    http://www.tcpdump.org/pcap.htm

    not sure precisely. Just I don't think I get your question precisely.anyway
    see around it may be useful or ignore if offtopic.

    Pegado de <http://www.daniweb.com/software-development/cpp/threads/264200>

    Where C and C++ Differ
    http://www.cprogramming.com/tutorial/c-vs-c++.html

    C99 versus C++98
    http://david.tribble.com/text/cdiffs.htm#C99-vs-CPP98

    A Library of Software written in C++ with full source code.
    http://cplus.about.com/od/codelibraryfor1/A_Library_of_Software_written_in_C_with_full_source_code.htm

    A Tutorial to Teach You How to Program in C++
    http://cplus.about.com/od/learning1/a/cpptutorial.htm

    10 Major Differences Between C And C++
    http://www.durofy.com/programming/10-major-differences-between-c-and-c/

Overriding Vs. virtual
    http://stackoverflow.com/questions/2238928/why-do-we-actually-have-virtual-functions

    http://www.parashift.com/c++-faq-lite/virtual-functions.html#faq-20.8

    http://publib.boulder.ibm.com/infocenter/lnxpcomp/v8v101/index.jsp?topic=%2Fcom.ibm.xlcpp8l.doc%2Flanguage%2Fref%2Fcplr139.htm

    http://stackoverflow.com/questions/2238928/why-do-we-actually-have-virtual-functions

    http://discuss.itacumens.com/index.php?topic=15513.0

    Librerías C++ (zator)
    http://www.zator.com/libreriasC.htm

googlemock Google C++ Mocking Framework
    http://code.google.com/p/googlemock/

googletest Google C++ Testing Framework
    http://code.google.com/p/googletest/wiki/Primer

10 Major Differences Between C And C++ | Durofy
    http://www.durofy.com/programming/10-major-differences-between-c-and-c/
